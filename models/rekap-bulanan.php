...
if (!empty($nama)) {
    ...
    $paramm .= 'bln=' . $bulan . '&thn=' . $tahun . '&nama=' . $nama . '';
} else {
    $paramm .= 'bln=' . $bulan . '&thn=' . $tahun . '';
}
...
$query = mysqli_query($con, "SELECT a.nama_lengkap, a.nama, a.nama_departemen, a.tgl_1, a.tgl_2, a.tgl_3, a.tgl_4, a.tgl_5, a.tgl_6, a.tgl_7, 
    ...
    AND c.id_departemen = d.id_departemen AND MONTH(a.tanggal) = '$bulan' AND YEAR(a.tanggal) = '$tahun' AND c.status = '1' $fltr  GROUP BY a.nama
...
<div style="background:#fff;height:400px;overflow:scroll;width:auto">
    <a href="<?php echo $BaseUrl; ?>/models/print-rekap-bulanan.php?<?php echo $paramm; ?>" target="_blank" style="float:right;margin-right:10px"><button class="btn btn-primary btn-sm"><i class="fa fa-print"></i> Print</button></a>
    <br>
    <hr>
    <center>
    ...
    <table class="table table-bordered table-sm">
        <tr>
            ...
            <th colspan="<?php echo $tanggal; ?>" style="background:#333;color:#fff;vertical-align:middle;text-align:center">Hari/Tanggal</th>
        ...
        <tr>
            ...
            for ($i = 1; $i < $tanggal + 1; $i++) {
                ...
                if ($dayList[$day] == 'Min' || $dayList[$day] == 'Sab') {
                    echo '<th style="background:red;color:#fff;text-align:center">' . $i . '</th>';
                } else {
            ...
            while ($data = mysqli_fetch_array($query)) {
                ...
                for ($i = 1; $i < $tanggal + 1; $i++) {
                    ...
                    if ($data['tgl_' . $i . ''] == "" && $dayList[$day] == 'Min' || $dayList[$day] == 'Sab') {
                        ...
                        if ($data['tgl_' . $i . ''] == 'T') {
                            echo '<td style="font-size:12px;background:orange;text-align:center">' . $data['tgl_' . $i . ''] . '</td>';
                        } else if ($data['tgl_' . $i . ''] == 'SG') {
                            echo '<td style="font-size:12px;background:#a5e5f2;text-align:center">' . $data['tgl_' . $i . ''] . '</td>';
                        } else if ($data['tgl_' . $i . ''] == 'C') {
                            echo '<td style="font-size:12px;background:#d9d5d7;text-align:center">' . $data['tgl_' . $i . ''] . '</td>';
                        } else if ($data['tgl_' . $i . ''] == 'S') {
                            echo '<td style="font-size:12px;background:#f2a4cc;text-align:center">' . $data['tgl_' . $i . ''] . '</td>';
                        } else if ($data['tgl_' . $i . ''] == 'P') {
                            echo '<td style="font-size:12px;background:#c7d91f;text-align:center">' . $data['tgl_' . $i . ''] . '</td>';
                        } else if ($data['tgl_' . $i . ''] == 'LK') {
                            echo '<td style="font-size:12px;background:#1fd95b;text-align:center">' . $data['tgl_' . $i . ''] . '</td>';
                        } else {
                            echo '<td style="font-size:12px;text-align:center">' . $data['tgl_' . $i . ''] . '</td>';
                        }
                    }
                }
                ...
                $query_spek = mysqli_query($con, "SELECT a.nama,IFNULL(b.total_telat,0) AS telat,
                            ...
                            IFNULL(g.total_lk,0) AS luar_kota FROM (
                        ...
                        ) AS g ON a.nama = g.nama ORDER BY a.nama ASC");
