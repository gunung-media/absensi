...
@section('content')
    <div style="margin-bottom: 10px">
        <a href="{{ route('admin.placement.create') }}"><button type="button" class="btn btn-default btn-sm"><i
                    class="fa fa-plus"></i> Tambah</button></a>
    </div>

    <div class="panel panel-primary">
        <div class="panel-heading">
            <i class="fa fa-th"></i> List Data Penggajian - Penempatan
        </div>
                    ...
                    <tbody>
                        @forelse ($placements as $index => $placement)
                            <tr>
                                ...
                                <td>{{ $placement->is_active ? 'Aktif' : 'Tidak Aktif' }}</td>
                                <td>
                                    <a href="{{ route('admin.placement.edit', $placement->id) }}"
                                        class="btn btn-primary btn-xs"><i class="fa fa-edit"></i> Edit</a>
                                    <form action="{{ route('admin.placement.destroy', $placement->id) }}" method="POST"
                                        class="d-inline delete-form" data-placement="{{ $placement->name }}">
                                        @csrf
                                        @method('DELETE')
